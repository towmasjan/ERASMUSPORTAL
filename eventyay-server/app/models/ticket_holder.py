...
class TicketHolder(SoftDeletionModel):
    ...
    # Erasmus+ Youth Exchange specific fields
    date_of_birth = db.Column(db.Date, nullable=True)
    special_diet_needs: str = db.Column(db.Text, nullable=True)
    health_issues: str = db.Column(db.Text, nullable=True)
    actual_travel_cost: float = db.Column(db.Float, default=0.0)
    emergency_contact_name: str = db.Column(db.String(255), nullable=True)
    emergency_contact_phone: str = db.Column(db.String(50), nullable=True)
    passport_number: str = db.Column(db.String(50), nullable=True)
    passport_expiry_date = db.Column(db.Date, nullable=True)
    nationality: str = db.Column(db.String(100), nullable=True)
    
    # Partner Organization relationship for Erasmus+
    partner_organization_id: int = db.Column(
        db.Integer, 
        db.ForeignKey('partner_organizations.id', ondelete='SET NULL'),
        nullable=True
    )
    ...
    def serialize(self):
        ...
        return {
            ...
            'gender': self.gender,
            'specialDietNeeds': self.special_diet_needs,
            'healthIssues': self.health_issues,
            'actualTravelCost': self.actual_travel_cost,
            'partnerOrganizationId': self.partner_organization_id,
