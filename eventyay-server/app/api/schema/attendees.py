...
class AttendeeSchemaPublic(SoftDeletionSchema):
    ...
    special_diet_needs = fields.Str(allow_none=True)
    health_issues = fields.Str(allow_none=True)
    actual_travel_cost = fields.Float(allow_none=True)
    emergency_contact_name = fields.Str(allow_none=True)
    emergency_contact_phone = fields.Str(allow_none=True)
    passport_number = fields.Str(allow_none=True)
    passport_expiry_date = fields.Date(allow_none=True)
    nationality = fields.Str(allow_none=True)
    
    # Partner Organization relationship for Erasmus+
    partner_organization = Relationship(
        self_view='v1.attendee_partner_organization',
        self_view_kwargs={'id': '<id>'},
        related_view='v1.partner_organization_detail',
        related_view_kwargs={'attendee_id': '<id>'},
